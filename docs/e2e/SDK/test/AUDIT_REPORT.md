...
| **Pool SDK Routing** | - | ✅ FIXED | **ISS-017** | Now joins with lead_assignments to get propensity_score |
...
| ISS-004 | HIGH | Data | ALS scores not calculated - propensity_score/als_tier columns exist but empty | PENDING: Need to run ALS scoring after enrichment |
| ISS-005 | CRITICAL | Code | admin.py:1176 queries propensity_score from lead_pool but column only exists in lead_assignments | FIXED: Changed query to use lead_assignments |
...
| ISS-007 | CRITICAL | Code | admin.py:1230 SELECT includes propensity_score, als_tier from lead_pool | FIXED: Removed columns, set to None in response |
...
| ISS-009 | CRITICAL | Code | reports.py:571-579 AVG(propensity_score) from lead_pool | FIXED: Changed to query lead_assignments |
...
| ISS-017 | HIGH | Code | content.py generate_sdk_email_for_pool can't route to SDK - propensity_score not available from lead_pool | FIXED: Added include_propensity_score param to _get_pool_lead, joins with lead_assignments |
...
- **content.py**: Fixed SDK routing to fetch propensity_score from lead_assignments
