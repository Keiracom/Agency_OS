...
function ComponentLoader({ name, color = "#00d4ff" }: { name: string; color?: string }) {
  return (
    <div className="w-full h-full min-h-[350px] bg-bg-cream/50 rounded-xl border border-slate-700/50 flex items-center justify-center">
      <div className="text-center">
        ...
        />
        <p className="text-ink-2 text-sm font-mono">Loading {name}...</p>
...
function PlaceholderCard({ name, color = "#666" }: { name: string; color?: string }) {
  return (
    <div className="w-full h-full min-h-[350px] bg-bg-cream/50 rounded-xl border border-dashed border-slate-600/50 flex items-center justify-center">
      <div className="text-center">
        <div
        ...
        <p className="text-ink-2 text-sm font-mono">{name}</p>
        <p className="text-ink-3 text-xs mt-1">Coming soon</p>
      </div>
    </div>
  );
}
  ...
  return (
    <>
        ...
        {isFullscreen && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-6xl h-[80vh] bg-bg-cream rounded-2xl border border-slate-700/50 overflow-hidden"
              ...
              <div className="h-full flex flex-col">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50">
                  <div>
                    ...
                    <p className="text-ink-3 text-sm font-mono">{engine}</p>
                  </div>
                  <button
                    onClick={() => setIsFullscreen(false)}
      ...
      <div className="group relative">
        ...
        {/* Card */}
        <div className="relative bg-bg-cream/80 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden h-full flex flex-col">
          {/* Header */}
          <div className="px-4 py-3 border-b border-slate-700/50 flex items-center justify-between flex-shrink-0">
            <div>
              ...
              <p className="text-ink-3 text-xs font-mono">{engine}</p>
            </div>
          ...
          <div className="h-[380px] flex-shrink-0 overflow-hidden">
            <AnimatePresence mode="wait">
              {showCode ? (
                ...
                >
                  <pre className="text-xs font-mono text-ink-2 bg-slate-950/50 p-4 rounded-lg overflow-x-auto">
          ...
          <div className="px-4 py-3 border-t border-slate-700/50 bg-slate-950/50 mt-auto">
            <p className="text-ink-3 text-xs leading-relaxed">{description}</p>
          </div>
        </div>
      </div>
    </>
...
export default function ShowroomPage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? "bg-slate-950" : "bg-bg-panel"}`}>
      {/* Animated Background */}
      ...
      <div className="relative z-10">
        {/* Header */}
        <header className={`border-b backdrop-blur-md sticky top-0 z-20 transition-colors duration-300 ${isDark ? "border-slate-800/50 bg-slate-950/80" : "border-gray-200 bg-bg-panel/80"}`}>
            ...
            <div className="flex items-center justify-between gap-8">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber to-amber flex items-center justify-center">
                    <span className="text-ink font-bold text-lg">S</span>
                  </div>
                  <div className={`absolute -bottom-1 -right-1 w-3 h-3 bg-amber rounded-full border-2 ${isDark ? "border-slate-950" : "border-white"}`} />
                </div>
                <div>
                  <h1 className={`text-xl font-bold ${isDark ? "text-ink" : "text-gray-900"}`}>Component Showroom</h1>
                  <p className={`text-xs font-mono ${isDark ? "text-ink-3" : "text-text-primary0"}`}>R3F • SPLINE • RIVE • UI EFFECTS</p>
                </div>
              </div>
              ...
              <div className="flex-1 max-w-md">
                <div className="relative">
                  ...
                  />
                  <svg className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${isDark ? "text-ink-3" : "text-ink-3"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className={`flex items-center gap-2 text-xs font-mono ${isDark ? "text-ink-3" : "text-text-primary0"}`}>
...
// Any color`}
                ...
                <div className="w-full h-full relative bg-slate-950">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <span className="text-ink/90 font-bold text-lg">Click for more!</span>
                    </div>
                  </div>
                </div>
              </ShowcaseCard>
        ...
        <section className="max-w-[1800px] mx-auto px-6 pb-12">
          <div className="bg-bg-cream/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6">
            <h2 className="text-ink font-semibold text-lg mb-6 flex items-center gap-3">
                  ...
                  <code className="text-ink-2 text-xs block space-y-1">
                    ...
                    {group.imports.map((imp, i) => (
                      <span key={imp} className="block pl-2">
                        ...
                        {i < group.imports.length - 1 && <span className="text-ink-3">,</span>}
                      </span>
                    ))}
                    <span className="text-ink-3">{"}"} from <span className="text-amber">&quot;@/src/components/library&quot;</span></span>
                  </code>
                </div>
              ))}
            </div>
