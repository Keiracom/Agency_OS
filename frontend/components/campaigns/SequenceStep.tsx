...
export function SequenceStep({ step }: Props) {
  return (
    <div className={`flex-1 p-3 rounded-lg text-center ${stepStyles[step.status]}`}>
      <div className="text-[10px] text-slate-500 uppercase">Day {step.day}</div>
      <div className="text-base my-1">{channelEmoji[step.channel]}</div>
      <div className="text-[11px] font-semibold text-slate-600">{step.label}</div>
      <div className="text-[10px] text-slate-500 mt-1">{step.stats}</div>
    </div>
  );
}
