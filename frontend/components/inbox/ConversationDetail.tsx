...
function AISuggestedReply({ suggestedReply }: { suggestedReply: string }) {
  return (
    <div className="bg-bg-panel border border-slate-200 rounded-2xl p-5 mt-auto">
      <div className="flex items-center gap-2 mb-3">
        <span className="px-2.5 py-1 bg-violet-100 text-amber rounded-md text-[11px] font-semibold">
          AI
        </span>
        <span className="text-sm font-semibold text-ink-3">Suggested Reply</span>
      </div>
      
      <div className="text-sm leading-relaxed text-ink-3 p-4 bg-slate-50 rounded-lg mb-4 whitespace-pre-wrap">
        {suggestedReply}
      </div>
...
export function ConversationDetail({ conversation }: ConversationDetailProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      {/* Header */}
      <div className="px-6 py-5 bg-bg-panel border-b border-slate-200">
