    ...
    <div className="rounded-xl bg-[#0a0a0f] border border-white/10 p-5">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex-1">
          <input type="text" value="https://yourwebsite.com.au" disabled className="w-full bg-transparent text-text-primary border-b border-white/20 pb-2 text-sm" />
        </div>
      </div>
      <div className="space-y-2.5">
        {["Industries: Healthcare, Professional Services, Real Estate", "Company size: 50-500 employees", "Titles: CMO, Marketing Director, Head of Growth"].map((item, i) => (
          <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg bg-bg-surface/5">
            <Check className="w-4 h-4 text-amber shrink-0" />
            <span className="text-xs text-text-primary/80">{item}</span>
          </div>
        ))}
      </div>
    </div>
...
function ScoutDemo() {
  ...
  return (
    <div className="rounded-xl bg-[#0a0a0f] border border-white/10 p-5 space-y-2.5">
      {leads.map((lead, i) => (
        <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg bg-bg-surface/5">
          <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${lead.gradient} flex items-center justify-center text-text-primary text-xs font-bold shrink-0`}>
            {lead.initials}
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm text-text-primary truncate">{lead.name}</p>
            <p className="text-xs text-text-primary/50 truncate">{lead.title}</p>
          </div>
    ...
    <div className="rounded-xl bg-[#0a0a0f] border border-white/10 p-5">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-amber to-amber-light flex items-center justify-center text-text-primary text-lg font-bold">92</div>
          <div>
            <p className="font-medium text-text-primary text-sm">Sarah Chen</p>
        ...
        {dimensions.map((dim, i) => (
          <div key={i}>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-text-primary/70">{dim.score}</span>
            </div>
...
function ReachDemo() {
  const steps = [
    ...
    { icon: Phone, label: "Voice AI call", sub: "Friendly follow-up", status: "● In progress", statusColor: "text-text-secondary", iconBg: "bg-amber/20", iconColor: "text-amber" },
  ];
  return (
    <div className="rounded-xl bg-[#0a0a0f] border border-white/10 p-5">
...
function ConvertDemo() {
  return (
    <div className="rounded-xl bg-[#0a0a0f] border border-white/10 p-5">
      ...
      <div className="space-y-3">
        <div className="p-3 rounded-lg bg-bg-surface/5">
          <p className="text-xs text-text-primary/40 mb-2">What converted this lead:</p>
          <div className="flex flex-wrap gap-1.5">
            <span className="text-xs px-2 py-0.5 rounded bg-bg-elevated/20 text-amber-light">Case study mention</span>
        ...
        <div className="p-3 rounded-lg bg-bg-surface/5">
          <p className="text-xs text-text-primary/40 mb-1">Pipeline impact:</p>
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-4 text-text-primary">From zero to booked meetings</h2>
        ...
        <div ref={containerRef} className="relative">
          ...
          <button
            onClick={goPrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-6 z-20 w-12 h-12 rounded-full bg-bg-surface/10 hover:bg-bg-surface/20 border border-white/20 flex items-center justify-center transition-all duration-300 backdrop-blur-sm"
          ...
          <button
            onClick={goNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-6 z-20 w-12 h-12 rounded-full bg-bg-surface/10 hover:bg-bg-surface/20 border border-white/20 flex items-center justify-center transition-all duration-300 backdrop-blur-sm"
                    ...
                    <div className="p-6 md:p-10">
                      ...
                      <div className="grid md:grid-cols-2 gap-8 items-center">
                        ...
                        <div>
                          ...
                          <h3 className="text-2xl md:text-3xl font-bold mb-4 text-text-primary">{slide.title}</h3>
                          <p className="text-text-primary/60 leading-relaxed mb-4">{slide.description}</p>
                          <p className="text-text-primary/40 text-sm">{slide.subtext}</p>
                        </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${activeIndex === index
                    ...
                    : "w-2 bg-bg-surface/20 hover:bg-bg-surface/40"
                  }
                `}
