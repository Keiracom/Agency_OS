      ...
      <form onSubmit={handleSubmit} className={`space-y-4 ${className}`}>
        <div>
          <input
            ...
            required
            className={`w-full h-12 px-4 rounded-xl border bg-bg-surface text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition ${
        ...
        <div>
          <input
            ...
            className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-bg-surface text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition"
            disabled={status === "loading"}
          />
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className={`flex flex-col sm:flex-row gap-3 ${className}`}>
        <div className="flex-1 relative">
          <input
            ...
            className={`w-full px-5 py-3 rounded-full border bg-bg-surface text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition-all ${
              status === "error" ? "border-amber-light" : "border-gray-200"
            }`}
            disabled={status === "loading"}
          />
        ...
        <button
          type="submit"
          disabled={status === "loading"}
          className="px-6 py-3 rounded-full bg-gray-900 text-text-primary font-medium hover:bg-gray-800 transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
        >
          {status === "loading" ? (
            <span className="flex items-center gap-2">
    ...
    <form onSubmit={handleSubmit} className={className}>
      <div className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto mb-4">
        <input
          ...
          placeholder="Enter your work email"
          className={`flex-1 px-6 py-4 rounded-full border bg-bg-surface text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition-all ${
            status === "error" ? "border-amber-light" : "border-gray-200"
          }`}
          disabled={status === "loading"}
        />
        <button
          type="submit"
          disabled={status === "loading"}
          className="px-8 py-4 rounded-full bg-gray-900 text-text-primary font-medium hover:bg-gray-800 transition-all hover:scale-105 hover:shadow-xl hover:shadow-black/10 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
        >
          {status === "loading" ? (
            <span className="flex items-center gap-2">
              <svg className="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24">
