...
function SourceBadge({ source }: { source: string | null }) {
  ...
  return (
    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-bg-elevated text-text-muted border border-border-subtle">
      {label}
    </span>
  );
}
  ...
  if (!hasDM) {
    return (
      <div className="p-3 rounded-lg bg-bg-elevated border border-border-subtle">
        <p className="text-xs text-text-muted font-mono">DM not identified</p>
      </div>
    );
  }

  return (
    <div className="rounded-lg bg-bg-elevated border border-border-subtle overflow-hidden">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-border-subtle flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {card.dm_title && (
            <div className="text-xs text-text-muted truncate">{card.dm_title}</div>
          )}
        </div>
      </div>
      ...
      <div className="px-3 py-2 space-y-1.5">
        ...
        {card.dm_email && (
          <div className="flex items-center gap-2">
            <Mail className="w-3.5 h-3.5 text-text-muted flex-shrink-0" strokeWidth={1.5} />
        ...
        {card.dm_mobile && (
          <div className="flex items-center gap-2">
            <Phone className="w-3.5 h-3.5 text-text-muted flex-shrink-0" strokeWidth={1.5} />
            <span className="text-xs font-mono text-text-secondary">{card.dm_mobile}</span>
          </div>
        )}
        ...
        {card.dm_linkedin_url && (
          <div className="flex items-center gap-2">
            <Linkedin className="w-3.5 h-3.5 text-text-muted flex-shrink-0" strokeWidth={1.5} />
      ...
      <div className="px-3 pb-3 flex gap-2">
        {card.dm_email && (
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-mono rounded bg-bg-surface border border-border-subtle text-text-secondary hover:text-amber hover:border-amber/30 transition-colors"
          >
