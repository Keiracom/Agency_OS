  ...
  return (
    ...
    >
      ...
      <div className="w-20 shrink-0">
        <p className="text-xs text-ink/40">{dayPart}</p>
        <p className="text-sm font-medium text-ink">{timePart}</p>
      </div>

      {/* Divider */}
      <div className="w-px h-10 bg-bg-panel/10" />

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className="font-medium text-ink truncate">{leadName}</p>
        {company && (
          <p className="text-sm text-ink/50 truncate">{company}</p>
        )}
      </div>
      ...
      <div className="flex flex-col items-end gap-1">
        ...
        <span className="text-xs text-ink/40">{duration} min</span>
      </div>
    </div>
  );
}
