  ...
  return (
    ...
    >
      ...
      <div className="w-20 shrink-0">
        <p className="text-xs text-text-primary/40">{dayPart}</p>
        <p className="text-sm font-medium text-text-primary">{timePart}</p>
      </div>

      {/* Divider */}
      <div className="w-px h-10 bg-bg-surface/10" />

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className="font-medium text-text-primary truncate">{leadName}</p>
        {company && (
          <p className="text-sm text-text-primary/50 truncate">{company}</p>
        )}
      </div>
      ...
      <div className="flex flex-col items-end gap-1">
        ...
        <span className="text-xs text-text-primary/40">{duration} min</span>
      </div>
    </div>
  );
}
