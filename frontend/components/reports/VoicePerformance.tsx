      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          {[
            ...
            <div key={stat.label} className="bg-bg-cream rounded-lg p-4 text-center">
              <p className="text-2xl font-bold font-mono text-ink">{stat.value}</p>
              <p className="text-[10px] text-ink-3 uppercase tracking-wider mt-1">{stat.label}</p>
            </div>
          ))}
        </div>
        {/* Objections Table */}
        <div className="space-y-2">
          {objectionData.map((obj) => (
            <div key={obj.objection} className="flex items-center justify-between bg-bg-cream rounded-md px-3 py-2.5">
              <span className="text-xs text-ink-2">{obj.objection}</span>
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono text-ink-3">{obj.count}x</span>
