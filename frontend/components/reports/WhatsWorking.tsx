      ...
      <div className="p-5">
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div className="bg-bg-cream rounded-lg p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-ink-3 mb-2">Who Converts</p>
          ...
          {/* Best Timing */}
          <div className="bg-bg-cream rounded-lg p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-ink-3 mb-2">Best Timing</p>
            {bestTiming.map((item) => (
              <div key={item.label} className="flex justify-between items-center py-1.5">
                <span className="text-xs text-ink-2">{item.label}</span>
                <span className="text-xs font-semibold font-mono text-[#22C55E]">{item.value}</span>
        ...
        <div className="p-3 bg-gradient-to-r from-[#D4956A]/10 to-[#3B82F6]/10 border border-[#D4956A]/30 rounded-lg">
          ...
          <p className="text-[13px] text-ink leading-relaxed">{discoveryInsight.text}</p>
        </div>
      </div>
    </div>
  );
