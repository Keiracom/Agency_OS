...
async function vercelRequest(endpoint: string, method = "GET", body?: object) {
  if (!VERCEL_TOKEN) {
  ...
  try {
    switch (name) {
      ...
      case "list_env_vars": {
        ...
        const envVars = data.envs.map((e: Record<string, unknown>) => ({
          key: e.key,
          target: e.target,
          type: e.type,
      ...
      case "promote_to_production": {
        ...
        const result = await vercelRequest(
          `/v10/projects/${deployment.projectId}/promote/${deploymentId}`,
          "POST"
        );
