...
## PHASE 18-A: Scraper Waterfall Architecture (NEW)
...
**Discovered:** During M1 testing - dilatedigital.com.au failed (Cloudflare timeout)
...
| Task | Description | Status | Files |
|------|-------------|--------|-------|
| SCR-001 | Create URL validator engine | 🔴 | `src/engines/url_validator.py` |
...
| SCR-006 | Add Camoufox to Railway Dockerfile | 🔴 | `Dockerfile` |
| SCR-007 | Manual fallback UI page | 🔴 | `frontend/app/onboarding/manual-entry/page.tsx` |
| SCR-008 | Add proxy configuration env vars | 🔴 | Railway env vars |
| SCR-009 | Scraper waterfall tests | 🔴 | `tests/test_engines/test_scraper_waterfall.py` |

### Implementation Order

1. **Phase A (Now):** SCR-001, SCR-002, SCR-003 — Fix URL validation + Apify waterfall
2. **Phase B (Now):** SCR-007 — Manual fallback UI (100% coverage)
3. **Phase C (Later):** SCR-004, SCR-005, SCR-006, SCR-008 — Camoufox integration
4. **Phase D (Later):** SCR-009 — Tests

### Cost Projection (8,000 agencies)

| Scenario | Tier Distribution | Total Cost |
|----------|-------------------|------------|
...
**Root cause discovered:** Test used wrong URL (`dilatedigital.com.au`) — actual site is `dilate.com.au` (standard WordPress, no Cloudflare). URL validator (Tier 0) would have caught this by following redirects.
