...
async def main():
    ...
    try:
        ...
        # ─────────────────────────────────────────── S2
        t0 = time.time()
        log.info("--- S2: GMB Reverse Lookup (batch_size=50) ---")
        try:
            stage2 = Stage2GMBLookup(gmb_client, conn)
            result2 = await stage2.run(batch_size=50)
        ...
        # ─────────────────────────────────────────── S3
        t0 = time.time()
        log.info("--- S3: DFS Profile (batch_size=50) ---")
        try:
            stage3 = Stage3DFSProfile(dfs_client, signal_repo, conn, delay=0.2)
            result3 = await stage3.run(VERTICAL, batch_size=50)
