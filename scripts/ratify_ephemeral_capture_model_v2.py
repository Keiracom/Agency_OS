...
this rewrites its value so downstream agents (Nova backfill #1278, future Face
spawns) inherit the ratified model.

Idempotent: re-running upserts the same value. Must be run by an allowlisted
callsign (elliot|dave) per the KEI-87 ceo_memory write-guard — orion is
...
NEW_VALUE = {
    ...
    "capture_loop": [
        ...
        "Agent (the Face) writes the decision DIRECTLY to the Hindsight fleet_decisions bank as "
    ...
    "critical_rule": (
        "Nothing writes automatically. The Face MUST call classify_and_save at every conversation "
...
def main() -> int:
    ...
    parser.add_argument("--callsign", default="elliot", help="allowlisted writer (elliot|dave)")
