...
def classify_au_phone(raw: str) -> PhoneClassification:
    ...
    if local.startswith(("02", "03", "07", "08")) or e164.startswith(
        ("+612", "+613", "+617", "+618")
    ):
        return PhoneClassification(
            raw=raw,
    ...
    if local.startswith(("1300", "1800", "13")):
        return PhoneClassification(
            raw=raw,
            normalized_e164=local,  # service numbers don't have E.164
            phone_type="service_number",
            sms_ok=False,
            voice_ai_ok=False,
        )
