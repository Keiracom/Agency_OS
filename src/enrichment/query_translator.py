...
class QueryTranslator:
    ...
    async def execute_maps_queries(
        self, config: CampaignConfig, keywords: list[str], suburbs: list[str], max_queries: int
    ) -> list[DiscoveryResult]:
    ...
    async def _log_query(
        ...
        try:
            await (
                self.supabase.table("discovery_queries")
                .insert(
                    {
    ...
    async def run(self, config: CampaignConfig) -> list[DiscoveryResult]:
        ...
        logger.info(
            "discovery_complete", total=len(results), passed=len(passed_results), mode=mode.value
        )
