...
class Cycle(Base, TimestampMixin):
    ...
    id: Mapped[UUID] = mapped_column(
        UUID_DB(as_uuid=True), primary_key=True, server_default="gen_random_uuid()"
    )
    client_id: Mapped[UUID] = mapped_column(
        UUID_DB(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False
    )
    ...
    cycle_day_1_date: Mapped[date] = mapped_column(
    ...
    # Relationships
    prospects: Mapped[list[CycleProspect]] = relationship(
        "CycleProspect", back_populates="cycle", cascade="all, delete-orphan"
    )
    actions: Mapped[list[OutreachAction]] = relationship(
        "OutreachAction", back_populates="cycle", cascade="all, delete-orphan"
    )
    events: Mapped[list[CycleEvent]] = relationship(
        "CycleEvent", back_populates="cycle", cascade="all, delete-orphan"
    )
...
class CycleProspect(Base, TimestampMixin):
    ...
    cycle_id: Mapped[UUID] = mapped_column(
        UUID_DB(as_uuid=True), ForeignKey("cycles.id", ondelete="CASCADE"), nullable=False
    )
    ...
    actions: Mapped[list[OutreachAction]] = relationship(
        "OutreachAction", back_populates="cycle_prospect", cascade="all, delete-orphan"
    )
...
class OutreachAction(Base):
    ...
    cycle_prospect_id: Mapped[UUID] = mapped_column(
        UUID_DB(as_uuid=True), ForeignKey("cycle_prospects.id", ondelete="CASCADE"), nullable=False
    )
    prospect_id: Mapped[UUID] = mapped_column(UUID_DB(as_uuid=True), nullable=False)
