    ...
    def score(self, enrichment: dict) -> AffordabilityResult:
        ...
        if any(t in entity_type for t in _NON_SMB_TYPES):
            return AffordabilityResult(
                raw_score=0,
                band="LOW",
                signals={"hard_gate": "public_or_govt"},
                gaps=[GAP_MESSAGES["public_or_govt"]],
                passed_gate=False,
            )
        ...
        has_booking = any(
            b in (enrichment.get("website_cms") or "").lower()
            or any(b in t.lower() for t in tech)
            or any(b in t.lower() for t in tracking)
