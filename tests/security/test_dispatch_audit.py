...
def test_fingerprint_first_twelve_hex_of_sha256():
    ...
    secret = "abc123-shared-secret"
    expected = hashlib.sha256(secret.encode("utf-8")).hexdigest()[:12]
    assert fingerprint(secret) == expected
...
def test_emit_audit_no_dsn_returns_false_no_raise(monkeypatch):
    ...
    rc = emit_audit("sign", result="ok", payload_id="x", target="atlas",
                    actor="elliot", secret_fingerprint="abc123abc123")
    assert rc is False
...
def test_emit_audit_unreachable_db_returns_false_no_raise(monkeypatch):
    ...
    rc = emit_audit("verify", result="mismatch", payload_id="x", target="atlas",
                    actor="unknown", reason="HMAC mismatch")
    assert rc is False
