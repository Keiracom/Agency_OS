...
class VoiceAgentService:
    ...
    async def process_post_call(
        ...
        Handles booking confirmations, follow-ups, and unsubscribe requests.
        """
...
class TestCallingHoursCompliance:
    ...
    async def test_calling_hours_uses_prospect_local_timezone(self, mock_lead_pool):
        ...
        utc_time = datetime(2025, 2, 18, 17, 0, tzinfo=timezone.utc)
...
class TestAdditionalRequirements:
    """Additional tests required per CEO directive."""

    @pytest.mark.asyncio
    async def test_post_call_processor_escalation_notifies_agency_owner(
        self, mock_lead_pool, mock_agency
    ):
        """
        Test that ESCALATION/ANGRY outcomes notify agency owner.
        
        Validates:
        - Escalation transcript triggers notification
        - escalation_notified = true on voice_calls record
        - Push notification sent to agency owner dashboard
        """
        service = VoiceAgentService()
        
        transcript = """
        Agent: I'm calling from Agency OS about your marketing needs.
        Prospect: This is ridiculous! I've told you people before not to call!
        I want to speak to your manager right now!
        Agent: I'm sorry for any frustration. Let me escalate this.
        """
        
        result = await service.process_post_call(
            lead=mock_lead_pool,
            transcript=transcript,
            call_metadata={"duration": 90, "call_id": "call_escalate"},
        )
        
        # Verify outcome
        assert result.outcome in [CallOutcome.ESCALATION, CallOutcome.ANGRY]
        
        # Verify escalation flag set
        assert result.escalation_notified is True
        
        # Verify notification sent (check notification service was called)
        assert "agency_notification" in result.actions_taken
    ...
    async def test_als_score_updated_on_booked_outcome(
        ...
        """
        Test that ALS score increases by +15 on BOOKED outcome.
        
        Validates:
        - BOOKED outcome triggers ALS adjustment
        - lead_pool.als_score increases by +15
        - ALS adjustment logged in conversion_events
        """
        initial_als = mock_lead_pool["als_score"]
        
        service = VoiceAgentService(supabase_client=mock_supabase_client)
        
        transcript = """
        Agent: Would a 15-minute call work for you next Tuesday?
        Prospect: Yes, that sounds great. Let's do 10am.
        ...
        result = await service.process_post_call(
            lead=mock_lead_pool,
            transcript=transcript,
            call_metadata={"duration": 180, "call_id": "call_booked"},
        )
        
        # Verify outcome
        assert result.outcome == CallOutcome.BOOKED
        
        # Verify ALS adjustment
        assert result.als_adjustment == 15
        
        # Verify lead_pool would be updated
        expected_als = initial_als + 15
        assert expected_als == mock_lead_pool["als_score"] + 15

    @pytest.mark.asyncio
    async def test_webhook_signature_verification_rejects_invalid(self):
        """
        Test that webhook rejects invalid signatures.
        
        Validates:
        - Invalid signature returns 401/403
        - Call is NOT processed
        - Security audit log entry created
        """
        from src.api.webhooks.elevenagets import verify_webhook_signature
        
        # Create test payload
        payload = {"call_id": "call_123", "status": "completed"}
